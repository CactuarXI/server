...
spellObject.onSpellCast = function(caster, target, spell)
    ...
    if debuff ~= nil and debuff:getSubPower() == xi.mod.FIRE_MEVA then
        ...
        target:addStatusEffectEx(xi.effect.NINJUTSU_ELE_DEBUFF, 0, power, 0, duration, 0, xi.mod.FIRE_MEVA, 0)
        ...
        target:addStatusEffectEx(xi.effect.NINJUTSU_ELE_DEBUFF, 0, 15, 0, 60, 0, xi.mod.FIRE_MEVA, 0)
        -- caster:printToPlayer(string.format('Duration: %s', duration), xi.msg.channel.SYSTEM_3)
        -- caster:printToPlayer(string.format('Power: %s', power), xi.msg.channel.SYSTEM_3)
    end
