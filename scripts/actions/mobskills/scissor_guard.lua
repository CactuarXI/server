...
mobskillObject.onMobWeaponSkill = function(target, mob, skill)
    ...
    if mob:isPet() then
        ...
        if master and master:hasJugPet() and master:checkDistance(mob) < 8.5 then
            ...
            duration = 60
            duration = math.max(60, duration * (tp/1000)) -- Minimum 1 minutes. Maximum 3 minutes.
            master:addStatusEffect(xi.effect.DEFENSE_BOOST, 100, 0, duration)
            mob:injectActionPacket(master:getID(), 11, 189, 0, 0, 0, 10, 1)
    ...
    if
        mob:getZone():getTypeMask() == xi.zoneType.DYNAMIS
    then
