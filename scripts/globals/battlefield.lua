...
local function isBCNMOrb(itemId)
    for _, orb in ipairs(xi.battlefield.bcnmOrbs) do
        if itemId == orb then
            return true
        end
    end
...
function Battlefield:onBattlefieldEnter(player, battlefield)
    ...
    then
        ...
        if isBCNMOrb(itemId) then
            player:delItem(self.requiredItems[1], 1, xi.inv.INVENTORY)
            player:messageSpecial(self.requiredItems.wearMessage, itemId, 1, 0)
        elseif totalUses > 1 then
