...
entity.onMobSpawn = function(mob)
    ...
    mob:setLocalVar('lifeTimer', os.time() + math.random(5, 10))
...
entity.onMobFight = function(mob, target)
    ...
    then
        mob:useMobAbility(2235 + math.random(0, 2))
...
entity.onMobRoam = function(mob, target)
    if
        mob:getLocalVar('lifeTimer') < os.time() and
        mob:getLocalVar('usedPayload') == 0
    then
        mob:setModelId(2224)
        mob:setHP(0)
