...
entity.onTrigger = function(player, npc)
    ...
    elseif
        aBoysDream == xi.questStatus.QUEST_COMPLETED and underOath == xi.questStatus.QUEST_AVAILABLE and
        ((player:getMainJob() == xi.job.PLD and mLvl >= xi.settings.main.AF2_QUEST_LEVEL) or 
        (player:getSubJob() == xi.job.PLD and sLvl >= xi.settings.main.AF2_QUEST_LEVEL))
    then
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 88 then
        if player:getFreeSlotsCount() == 0 then
            ...
            if
                player:getMainJob() == xi.job.PLD or
                player:getSubJob() == xi.job.PLD
            then
