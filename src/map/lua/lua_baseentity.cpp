...
void CLuaBaseEntity::sendEmote(CLuaBaseEntity* target, uint8 emID, uint8 emMode, bool self)
{
    ...
    if (target)
    {
        ...
        if (PChar && PTarget)
        {
            ...
            //if (self)
            //{
            //    PChar->PushPacket(PChar, CHAR_INRANGE, std::make_unique<CCharEmotionPacket>(PChar, PTarget->id, PTarget->targid, emoteID, emoteMode, 0));
            //}
        }
    }
}
...
auto CLuaBaseEntity::addSoulPlate(std::string const& name, uint8 fauna, uint8 subOfInterest, uint8 ecoSystem, uint8 zeni, uint16 skillIndex, uint8 fp) -> CItem*
// LSB: auto CLuaBaseEntity::addSoulPlate(std::string const& name, uint16 mobFamily, uint8 zeni, uint16 skillIndex, uint8 fp) -> CItem*
{
    if (m_PBaseEntity->objtype != TYPE_PC)
    {
        ShowWarning("Invalid entity type calling function (%s).", m_PBaseEntity->getName());
...
void CLuaBaseEntity::sendUpdateToZoneCharsInRange(float maxDistance = 0.0f)
{
    ...
    for (EntityList_t::const_iterator it = charList.begin(); it != charList.end(); ++it)
    {
        ...
        {
            PChar->pushPacket(std::make_unique<CEntityUpdatePacket>(m_PBaseEntity, ENTITY_SPAWN, UPDATE_ALL_MOB));
        }
    }
}
 ...
 *  Example : player:addQuest(xi.questLog.BASTOK, xi.quest.id.bastok.LURE_OF_THE_WILDCAT)
 ************************************************************************/
 ...
 *  Example : player:delQuest(xi.questLog.WINDURST, xi.quest.id.windurst.LURE_OF_THE_WILDCAT)
 ...
 *  Example : player:getQuestStatus(xi.questLog.WINDURST, xi.quest.id.windurst.MAKING_THE_GRADE)
 ************************************************************************/
 ...
 *  Example : if (player:hasCompletedQuest(xi.questLog.JEUNO, xi.quest.id.jeuno.BEYOND_INFINITY)) then
 ************************************************************************/
 ...
 *  Example : player:completeQuest(xi.questLog.OTHER_AREAS, xi.quest.id.otherAreas.ONLY_THE_BEST)
 *  Notes   :
 ************************************************************************/
...
CStatusEffect* CLuaBaseEntity::getItemEnchantmentEffect(sol::object const& ItemID)
{
    return getStatusEffect(EFFECT_ENCHANTMENT, ItemID, sol::lua_nil, sol::lua_nil);
}
...
/************************************************************************
 *  Function: countEffectBySubType()
 *  Purpose : Returns the number of Effects an Entity has in their container
 *  Example : if target:countEffect() > 3 then
 *  Notes   :
 ************************************************************************/

uint8 CLuaBaseEntity::countEffectBySubID(uint16 StatusID, uint32 SubID)
{
    if (m_PBaseEntity->objtype == TYPE_NPC)
    {
        ShowWarning("Invalid Entity (NPC: %s) calling function.", m_PBaseEntity->getName());
    ...
    auto effect_StatusID = static_cast<EFFECT>(StatusID);
    auto effect_SubID    = static_cast<uint32>(SubID);
    return PBattleEntity->StatusEffectContainer->GetEffectsCountBySubID(effect_StatusID, effect_SubID);
}
...
bool CLuaBaseEntity::addCorsairRoll(uint8 casterJob, uint8 bustDuration, uint16 effectID, uint16 power, uint32 tick, uint32 duration, sol::object const& arg6, sol::object const& arg7, sol::object const& arg8)
{
    ...
    // Umeboshi "Subjobs are not nerfed on cactuar. Max rolls will always be 2"
    ...
    switch (static_cast<Mod>(statId))
    {
        ...
        case Mod::RATT:
        {
            ...
            if (PEntity->objtype == TYPE_PET && static_cast<CPetEntity*>(PEntity)->getPetType() == PET_TYPE::AUTOMATON)
            {
                ...
                value = PEntity->RATT(skill, 0, 0, false);
            }
            else
            {
                ...
                if (PWeapon)
                {
                    value = PEntity->RATT(PWeapon->getSkillType(), 0, 0, false);
                }
                else
                {
                    value = PEntity->RATT(SKILL_MARKSMANSHIP, 0, 0, false); // TODO: does this edge case exist? will mobs or trusts hit this?
                }
            }
        }
        break;
...
float CLuaBaseEntity::getRangedPDIF(CLuaBaseEntity* PLuaBaseEntity, bool isCritical, float atkMulti, uint16 ignoredDef)
{
    ...
    return battleutils::GetRangedDamageRatio(PAttacker, PDefender, isCritical, atkMulti, ignoredDef, 0);
}
...
void CLuaBaseEntity::despawnFellow()
{
    ...
    if (PFellow != nullptr)
    {
        PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE_SELF, std::make_unique <CFellowDespawnPacket>(PFellow));
        PChar->RemoveFellow();
    }
}
...
void CLuaBaseEntity::sendNpcEmote(CLuaBaseEntity* PBaseEntity, sol::object const& p0, sol::object const& p1, sol::object const& p2)
{
    ...
    if (PNpc)
    {
        ...
        PNpc->loc.zone->PushPacket(PNpc, CHAR_INRANGE,
                                   std::make_unique <CCharEmotionPacket>(PNpc, EntityId, EntityIndex, emoteID, emoteMode, extra));
    }
}
...
void CLuaBaseEntity::sendMobEmote(CLuaBaseEntity* PBaseEntity, sol::object const& p0, sol::object const& p1, sol::object const& p2)
{
    ...
    if (PMob)
    {
        ...
        PMob->loc.zone->PushPacket(PMob, CHAR_INRANGE,
                                   std::make_unique <CCharEmotionPacket> (PMob, EntityId, EntityIndex, emoteID, emoteMode, extra));
    }
}
...
void CLuaBaseEntity::Register()
{
    ...
    SOL_REGISTER("countEffectBySubID", CLuaBaseEntity::countEffectBySubID);
