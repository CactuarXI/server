...
void CZoneEntities::FindPartyForMob(CBaseEntity* PEntity)
{
    ...
    bool      forceLink = PMob->getMobMod(MOBMOD_SUPERLINK);
        ...
        if (PChar->m_PFellow != nullptr)
        {
            ...
            for (EntityList_t::const_iterator it = m_charList.begin(); it != m_charList.end(); ++it)
            {
                ...
                if (Fellow != PCurrentChar->SpawnPETList.end())
                {
                    ...
                    PCurrentChar->pushPacket(std::make_unique<CEntityUpdatePacket>(PChar->m_PFellow, ENTITY_DESPAWN, UPDATE_NONE));
                }
            }
...
void CZoneEntities::SpawnMOBs(CCharEntity* PChar)
{
    ...
    FOR_EACH_PAIR_CAST_SECOND(CMobEntity*, PCurrentMob, m_mobList)
    {
        ...
        if (PCurrentMob->getMobMod(MOBMOD_PIXIE) > 0) // TODO: Check This
        {
