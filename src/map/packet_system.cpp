        ...
        case 0x00: // trigger
        {
            ...
            if (PChar->m_PFellow != nullptr && PChar->m_PFellow->targid == TargID)
            {
                if (data.ref<uint8>(0x0C) == 1)
                {
                    ...
                    PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE_SELF, std::make_unique<CFellowDespawnPacket>(PChar->m_PFellow));
                    PChar->RemoveFellow();
                }
                else
        ...
        case 0x13: // tractor menu
        {
            if (data.ref<uint8>(0x0C) == 0 && PChar->m_hasTractor != 0) // ACCEPTED TRACTOR
            {
                ...
                if (PChar->StatusEffectContainer->HasStatusEffect(EFFECT_BATTLEFIELD))
                {
                    ...
                    PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE_SELF, std::make_unique<CPositionPacket>(PChar));
...
void SmallPacket0x061(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket& data)
{
    ...
    PChar->pushPacket<CCharStatsPacket>(PChar, false); // Umeboshi "resetflips"
...
void SmallPacket0x084(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket& data)
{
    ...
    if (PChar->animation != ANIMATION_SYNTH && (PChar->CraftContainer && PChar->CraftContainer->getItemsCount() == 0))
    {
        ...
        if ((PItem != nullptr) && (PItem->getID() == itemID) && !(PItem->getFlag() & ITEM_FLAG_NOSALE))
        {
            ...
            PChar->pushPacket<CShopAppraisePacket>(slotID, basePrice * mult);
        }
        return;
    }
}
    ...
    if (type == 0x02)
    {
        if (PChar->id == id)
        {
            ...
            if (PChar->getEquip(SLOT_RANGED) && PChar->getEquip(SLOT_RANGED)->isType(ITEM_WEAPON))
            {
                ...
                PChar->pushPacket<CMessageBasicPacket>(PChar, PChar, PChar->RACC(skill, 0, bonusSkill, false), PChar->RATT(skill, 0, bonusSkill, false), MSGBASIC_CHECKPARAM_RANGE);
            }
        ...
        else if (PChar->PPet && PChar->PPet->id == id)
        {
            ...
            if (PChar->getEquip(SLOT_RANGED) && PChar->getEquip(SLOT_RANGED)->isType(ITEM_WEAPON))
            {
                ...
                PChar->pushPacket<CMessageBasicPacket>(PChar, PChar->PPet, PChar->PPet->RACC(skill, 0, 0, false), PChar->PPet->RATT(skill, 0, 0, false), MSGBASIC_CHECKPARAM_RANGE);
            }
            else if (PChar->getEquip(SLOT_AMMO) && PChar->getEquip(SLOT_AMMO)->isType(ITEM_WEAPON))
            {
                int skill = ((CItemWeapon*)PChar->getEquip(SLOT_AMMO))->getSkillType();
